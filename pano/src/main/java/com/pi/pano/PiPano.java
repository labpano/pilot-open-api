...
class PiPano
{
    public static final boolean sDebug = BuildConfig.PI_DEBUG;
    ...
    void setParamReCaliEnable(Object listener)
    {
        Message.obtain(mUpdateFrameTask.mHandler,MSG_PIPANO_PARAM_RECALI,listener).sendToTarget();
    }
    ...
    private static class FrameAvailableTask extends HandlerThread {
    ...
    private class UpdateFrameTask extends HandlerThread implements SurfaceTexture.OnFrameAvailableListener, Handler.Callback
    {
        ...
        private Bitmap makePreviewDebugTexture(String text) {
        ...
        private Bitmap makeMediaDebugTexture(String text, int color) {
            Bitmap bitmap = Bitmap.createBitmap(256,128, Bitmap.Config.ARGB_8888);
            Canvas canvas = new Canvas(bitmap);
            TextPaint textPaint = new TextPaint();
        ...
        public boolean handleMessage(Message msg)
        {
            switch (msg.what) {
                case MSG_PIPANO_INIT:
                    createNativeObj(PiPano.this.mPiPanoListener.getClass() != MediaPlayerSurfaceView.class, sDebug, mCacheDir);
                ...
                case MSG_PIPANO_UPDATE_PRE_SECOND:
                    ...
                    if (sDebug) {
                        ...
                        builder.append("\ngpu: ").append(String.format("%.2f", gpuUtilization));
                ...
                case MSG_PIPANO_PARAM_RECALI:
                    ParamReCaliListener paramReCaliListener = (ParamReCaliListener)msg.obj;
                    setParamReCaliEnableImpl(paramReCaliListener.mExecutionInterval, paramReCaliListener);
                    break;
                case MSG_PIPANO_FRAME_AVAILABLE:
                    ...
                    if (mPiPanoListener.getClass() == MediaPlayerSurfaceView.class||
                            mPiPanoListener.getClass() == StitchingThread.class) {
                        if (surfaceTexture == mSurfaceTexture[0]) {
                            Message.obtain(mHandler, MSG_PIPANO_ENCODER_SURFACE_UPDATE, surfaceTexture.getTimestamp()).sendToTarget();
                        }
                    }
                break;
                case MSG_PIPANO_ENCODER_SURFACE_UPDATE:
                    ...
                    if (sDebug)
                    {
                        ...
                        makeDebugTexture(false, makeMediaDebugTexture(builder.toString(), isFrameSync?Color.YELLOW:Color.RED),
                                0, 0.95f, 0.06f, 0.05f);
                    }
